...
int main(int argc, char **argv) {
   ...
   GLFWwindow *window =
      glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "NoobEngine", NULL, NULL);
   if (window == NULL) {
   ...
   projection = glm::perspective(45.0f, (GLfloat)SCR_WIDTH / (GLfloat)SCR_HEIGHT,
      0.1f, 100.0f);
   ...
   while (!glfwWindowShouldClose(window)) {
      ...
      // glm::mat4 model(1.0f);
      glm::mat4 view(1.0f);
      // model = glm::rotate(model, sin((float)glfwGetTime())*1.5f,
      // glm::vec3(0.5f, 0.5f, 0.5f)); view = glm::translate(view, glm::vec3(0.0f,
      // 0.0f, -2.0f));
      ...
      transform = glm::scale(transform, glm::vec3(0.9, 0.9,
      0.9)*sin((float)glfwGetTime())); transform = glm::rotate(transform,
      (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));*/
      /*unsigned int transformLoc = glGetUniformLocation(shader.GetShaderID(),
      ...
      for (GLuint i = 0; i < 10; ++i) {
         ...
         model = glm::rotate(model, sin((float)glfwGetTime()) * 0.2f * (i + 1),
            glm::vec3(1.0f, 1.0f, 1.0f));
         model = glm::scale(
            model, glm::vec3(1.2f, 1.2f, 1.2f)); // *sin((float)glfwGetTime()));
      ...
      glfwPollEvents();
      glClear(GL_COLOR_BUFFER_BIT |
         GL_DEPTH_BUFFER_BIT); // claer z-buff for next frame
   }
